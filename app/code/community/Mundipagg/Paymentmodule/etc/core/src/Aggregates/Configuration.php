...
namespace Mundipagg\Core\Aggregates;

use Exception;
use Mundipagg\Aggregates\IAggregateRoot;
use Mundipagg\Core\Kernel\GatewayKey\AbstractGatewayKey;
...
use Mundipagg\Core\Kernel\GatewayKey\TestSecretKey;
use Mundipagg\Core\Kernel\GatewayId\GUID;
use Mundipagg\ValueObject\CardConfig;
...
final class Configuration implements IAggregateRoot
{
     ...
     *
     */
    const KEY_SECRET_TEST = 'KEY_SECRET_TEST';
    const KEY_PUBLIC_TEST = 'KEY_PUBLIC_TEST';
    ...
    public function __construct()
    {
        ...
        $this->keys = [
            self::KEY_SECRET_TEST => null,
            self::KEY_PUBLIC_TEST => null,
    ...
    {
        if ($this->isTestMode()) {
            return $this->keys[self::KEY_PUBLIC_TEST];
        }
    ...
    {
        if ($this->isTestMode()) {
            return $this->keys[self::KEY_SECRET_TEST];
        }
    ...
    public function addCardConfig(CardConfig $newCardConfig)
    {
        if (!in_array($newCardConfig->getBrand(), self::$validBrands)) {
            throw new Exception("The brand is invalid! {$newCardConfig->getBrand()}");
        }

        foreach ($this->cardConfigs as $cardConfig) {
            if ($cardConfig->equals($newCardConfig)) {
                throw new Exception("The card config is already added! {$newCardConfig->getBrand()}");
            }
        }
